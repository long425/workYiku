Page({
  ...
  getBranchList() {
    ...
    dd.httpRequest({
      ...
      url: "http://"+port+"/u8cloud/api/uapbd/bddept/query",
      method: 'POST',
      data: JSON.stringify({}),
      dataType: 'json',
      success: res => {
        if(res.data.status === 'success'){
          var branchData = JSON.parse(res.data.data);
          that.setData({ branchList: branchData.datas });
        };
      }
    });
  },
  checkBranch(e) {
    ...
    DATA.parentvo.cdeptname = e.target.dataset.name;
    DATA.parentvo.cdeptid = e.target.dataset.code;
    dd.setStorage({ key: 'DATA', data: JSON.stringify(DATA), success: res => {
